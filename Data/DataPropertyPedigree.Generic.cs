		...
		where T : class
	{
		...
		public sealed class TreeNode : IReadOnlyShell<T>
		{
			...
			public sealed class ChildTreeNodeCollection : IReadOnlyList<TreeNode>
			{
				...
				internal ChildTreeNodeCollection(TreeNode owner)
				{
					this.owner = owner;
				}
		...
		public sealed class TreeBuilder
		{
			...
			internal TreeBuilder(DataPropertyPedigree<T> owner)
			{
				this.owner = owner;
		...
		#region class PedigreeOperator
		/// <summary>
		/// 系譜操作
		/// </summary>
		[System.Diagnostics.DebuggerDisplay("Count = {Count}")]
		[System.Diagnostics.DebuggerTypeProxy(typeof(DataPropertyPedigree<>.PedigreeOperator.DebugView))]
		public class PedigreeOperator : DictionaryOperatorBase, IDisposable
		{
			#region class TreeNode
			[System.Diagnostics.DebuggerDisplay("{Core}")]
			[System.Diagnostics.DebuggerTypeProxy(typeof(DataPropertyPedigree<>.PedigreeOperator.TreeNode.DebugView))]
			public sealed class TreeNode : IReadOnlyShell<T>
			{
				#region class ChildTreeNodeCollection
				[System.Diagnostics.DebuggerDisplay("Count = {Count}")]
				[System.Diagnostics.DebuggerTypeProxy(typeof(DataPropertyPedigree<>.PedigreeOperator.TreeNode.ChildTreeNodeCollection.DebugView))]
				public sealed class ChildTreeNodeCollection : IReadOnlyList<TreeNode>
				{
					/// <summary>
					/// 番号の検索
				...
				#region internal members
				internal TreeNode(PedigreeOperator owner, int index)
				{
					this.owner = owner;
					this.index = index;
				...
				internal TreeNode(PedigreeOperator owner)
				{
					this.owner = owner;
				...
				private PedigreeOperator owner;
				private int index;
				#endregion // private members
			}
			#endregion // class TreeNode
			...
			/// 樹形図の構築
			/// </summary>
			[System.Diagnostics.DebuggerTypeProxy(typeof(DataPropertyPedigree<>.PedigreeOperator.TreeBuilder.DebugView))]
			public sealed class TreeBuilder
			{
				public TreeNode Root => Nodes.Root;
				...
				internal class TreeNodeCache : IReadOnlyIndexer<int, TreeNode>
				{
					...
					private TreeNode[] PrepareItems()
					{
						if (this.items == null)
						{
							this.items = new TreeNode[this.owner.owner.Shells.Count + 1];
						}
				...
				internal TreeBuilder(PedigreeOperator owner)
				{
					this.owner = owner;
			...
			public void Parent(T item, T parent)
			{
				...
				var currentParent = property.Value;
				...
				if (parent != null)
				{
					...
					for (var sampleIndex = 0; sampleIndex < Shells.Count; ++sampleIndex)
					{
			...
			public bool Remove(T item)
			{
				...
				var parent = Shells[target].Value;
				for (var index = 0; index < Shells.Count; ++index)
				{
					if (Shells[index].Value == item)
					{
						SetItem(index, Shells[index].Key, parent);
					}
				}
			...
			#region internal members
			internal PedigreeOperator(DataPropertyPedigree<T> owner)
				: base(owner)
			{
			}
			#endregion // internal members
			...
			#region DictionaryOperatorBase virtual member override
			protected override void OnOperated(IEnumerable<DataPropertyDictionaryChangeOperation<T, T>> operations)
			{
				Tree.Nodes.Reset();
			}
			#endregion // DictionaryOperatorBase virtual member override
			...
			private class DebugView
			{
				public DebugView(PedigreeOperator target)
				{
					this.target = target;
				}
				...
				#region private members
				private PedigreeOperator target;
				#endregion // private members
			}
			#endregion // class DebugView
			...
			private bool EvaluateDescendant(T item, T parent)
			{
				...
				do
				{
					ancestor = Shells[ancestor].Value;
					if (ancestor == parent)
					{
						return true;
					}
		...
		#endregion // class PedigreeOperator
