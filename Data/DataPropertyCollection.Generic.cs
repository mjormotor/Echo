...
namespace Echo.Data
{
	...
	public class DataPropertyCollection<T> : IReadOnlyList<DataProperty<T>>, IReadOnlyIndexer<int, DataProperty<T>>, IDataPropertyCollection
	{
		...
		public class CollectionOperator : IReadOnlyList<T>, IDisposable
		{
			...
			#region IReadOnlyList<T> interface support
			#region IReadOnlyCollection<T> interface support
			#region IEnumerable<T> interface support
			...
			public void Dispose()
			{
				if (this.block != null)
				{
					...
					var e = new DataPropertyCollectionChangingEventArgs<T>(this.owner, this.operations);
					this.owner.OnDataPropertyCollectionChanging(e);
					if (!e.Cancel)
					{
						if (this.owner.Execute(e.Operations))
						{
							this.owner.OnDataPropertyCollectionChanged(new DataPropertyCollectionChangedEventArgs<T>(this.owner, e.Operations, e.InputOperations));
						}
					}
				}
			}
